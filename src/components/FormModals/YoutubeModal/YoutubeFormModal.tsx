...
const YoutubeFormModal = ({ recoilApi = () => {} }) => {
  ...
  const { data, loading, error } = useGetCall(SERVICE.YOUTUBECHANNELS, {
  ...
  return (
    <Modal
      title={`${Edit ? "Edit" : "Add"} YouTube Channel`}
      onClose={closeModal}
    >
      {loading ? (
        <Loader />
...
export default YoutubeFormModal;
