...
import DailyVideoForm from "./PromotionVideoForm";

const PromotionVideoFormModal = ({ recoilApi = () => {} }) => {
  ...
  } = useActionCall(SERVICE.PROMOTIONVIDEOS);
  const { data, loading, error } = useGetCall(SERVICE.PROMOTIONVIDEOS, {
  ...
  return (
    <Modal
      title={`${Edit ? "Edit" : "Add"} Promotion Video`}
      onClose={closeModal}
    >
      {loading ? (
        <Loader />
...
export default PromotionVideoFormModal;
